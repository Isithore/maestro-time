...
// ---- Save to Firestore ----
const saveTimetableToFirestore = async (timetableData: any) => {
  try {
    // Days for reference
    const DAYS = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday'];

    // Convert class schedules
    const classes = timetableData.classes.map((cls: any) => {
      const scheduleByDay: Record<string, any[]> = {};

      // Group flat schedule array by day
      cls.schedule.forEach((slot: any) => {
        ...
        scheduleByDay[slot.day].push({
          ...
          timeRange: slot.timeRange || `Period ${slot.period ?? 0}`,
          subject: slot.subject || "Unassigned",
          subjectCode: slot.subjectCode || "",
          staff: slot.staff || "Unassigned",
          className: slot.className || cls.className,
        });
      });
    ...
    // Convert staff schedules
    const staff = timetableData.staff.map((s: any) => {
      const scheduleByDay: Record<string, any[]> = {};
    ...
    const firestoreData = {
      ...
      data: {
        classes,
        staff,
        warnings: timetableData.warnings || [],
      },
    };

    await setDoc(doc(db, "timetables", "latest"), firestoreData);
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // Save to Firestore
      await saveTimetableToFirestore(timetableData);
      ...
      toast({
        title: "Error",
        description: "Failed to generate or save timetable. Please try again.",
        variant: "destructive"
      });
    }
  };
      ...
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <Card className="shadow-card">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
              <Badge variant="secondary">{selectedDepartments.length} Dept</Badge>
              ...
              <Badge variant="outline">8 Periods</Badge>
              <Badge variant="secondary">
                Total: {selectedDepartments.length * yearsPerDepartment * sectionsPerYear}
              </Badge>
            </div>
          </CardContent>
        </Card>
            ...
            {subjects.map((subject) => (
              <div key={subject.id} className="bg-muted/50 rounded-lg p-4 space-y-4">
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center space-x-2">
                      <Checkbox
                        ...
                        onCheckedChange={(checked) =>
                          updateSubject(subject.id, { isLab: !!checked, duration: checked ? 2 : 1 })
                        }
                      />
                  ...
                  <Button variant="outline" size="sm" onClick={() => removeSubject(subject.id)} className="ml-2">
                    ...
                    {subject.staff.map((staff, index) => (
                      <div key={index} className="flex gap-2">
                        ...
                        {subject.staff.length > 1 && (
                          <Button variant="outline" size="sm" onClick={() => removeStaffMember(subject.id, index)}>
                            <Trash2 className="h-4 w-4" />
                          </Button>
                        )}
                      </div>
        ...
        <div className="text-center">
          <Button
            ...
            size="lg"
            className="bg-gradient-primary hover:opacity-90 px-8 py-3 text-lg font-semibold shadow-elevated"
          >
